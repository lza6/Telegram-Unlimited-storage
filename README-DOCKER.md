...
`.env` 里 `COMPOSE_FILE=docker-compose.yml:docker-compose.dev.yml` 会自动合并 dev 层，**`docker compose up -d --build` 与 `dev.bat` / `up.bat` 同一套栈**（Web/docs Volume + Rust cargo watch）。
...
| 改 Rust 业务代码 `src/*.rs` | 保存 → cargo watch 自动重编 | **数十秒**（增量编译，无 docker build） |

| 改 `Cargo.lock` / Dockerfile | `.\scripts\dev-up.ps1 -Rebuild` | 较慢（仅此时 rebuild） |

| **生产/交付** 全量镜像 | `docker compose up -d --build` | 完整 release 构建 |

| 不用 dev compose 时的增量 build | `.\scripts\dev-build-rust.ps1` | 备选方案 |



**勿用 `latest` 标签**：基础镜像是 `rust:1.85-bookworm` / `debian:bookworm-slim`；构建脚本带 `--pull=false`。
...
cargo run --release --bin telegram-drive-server --features headless-server

```



或根目录 `start.bat`（自动读 `.env`）。
...
| 可选代理 | 在 `data/network_settings.json` 配置 SOCKS5（与桌面版相同） |
