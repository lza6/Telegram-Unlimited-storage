...
impl RedisChunkIndex {
    ...
    pub async fn get_chunks(&self, session_id: &str) -> Result<Vec<UploadChunkRecord>, String> {
        ...
        let chunks: Vec<UploadChunkRecord> = raw_chunks
            .iter()
            .filter_map(|s| {
...
pub fn build_chunk_index_backend(
    config: &ServerConfig,
    db: &DbConnection,
) -> ChunkIndexBackendEnum {
    ...
    match backend_type {
        ChunkIndexBackendType::Redis => {
            ...
            let prefix = std::env::var("CHUNK_INDEX_REDIS_PREFIX")
                .unwrap_or_else(|_| "td:chunk:".to_string());
...
mod tests {
    ...
    fn backend_type_from_env_defaults_to_sqlite() {
        assert_eq!(
            ChunkIndexBackendType::from_env(),
            ChunkIndexBackendType::Sqlite
        );
    }
}
