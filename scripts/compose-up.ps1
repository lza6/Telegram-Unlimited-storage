# 统一 Docker 启动/更新：前后端同一套 compose（web/docs Volume + Rust cargo watch）
# 日常：dev.bat 或 docker compose up -d
# 全量更新：dev.bat -Build 或 docker compose up -d --build
param(
    [switch]$Build,
    [switch]$Rebuild,
    [switch]$Sync,
...
# 供用户在 shell 里直接敲 docker compose …（Windows 用 ; Linux/macOS 用 :）
if ($Release) {
...
try {
    ...
    if ($Sync) {
        ...
        if ($Rebuild) {
            Write-Host "Rebuilding image (Cargo.lock / Dockerfile changed) ..." -ForegroundColor Yellow
    ...
    $port = Get-TdPort
    $mode = if ($Release) { "release" } else { "dev (cargo watch)" }
    Write-Host ""
    ...
    if (-not $Release) {
        Write-Host "  Rust:         cargo watch in container (or sync.bat restart)" -ForegroundColor DarkGray
    }
    Write-Host "  Logs:         docker compose logs -f telegram-drive-api" -ForegroundColor DarkGray
    Write-Host "  Full rebuild: only Cargo.lock/Dockerfile -> dev.bat -Rebuild" -ForegroundColor DarkGray
    Write-Host ""
