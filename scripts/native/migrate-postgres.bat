@echo off
setlocal EnableExtensions DisableDelayedExpansion
for %%I in ("%~dp0..\..") do set "ROOT=%%~fI"
set "PG_BIN=C:\Program Files\PostgreSQL\16\bin"
set "PSQL=%PG_BIN%\psql.exe"
set "ENV_FILE=%ROOT%\.env"
if not exist "%PSQL%" (echo PostgreSQL 16 client tools are required.& exit /b 1)
if not exist "%ENV_FILE%" (echo .env is missing. Copy .env.example and complete the PostgreSQL settings.& exit /b 1)

for /f "usebackq tokens=1,* delims==" %%A in ("%ENV_FILE%") do (
  if "%%A"=="POSTGRES_PASSWORD" set "PGPASSWORD=%%B"
  if "%%A"=="POSTGRES_USER" set "PGUSER=%%B"
  if "%%A"=="POSTGRES_DB" set "PGDATABASE=%%B"
  if "%%A"=="POSTGRES_APP_USER" set "POSTGRES_APP_USER=%%B"
  if "%%A"=="POSTGRES_APP_PASSWORD" set "POSTGRES_APP_PASSWORD=%%B"
  if "%%A"=="SAGA_OPERATOR_ID" set "SAGA_OPERATOR_ID=%%B"
  if "%%A"=="SAGA_OPERATOR_TOKEN" set "SAGA_OPERATOR_TOKEN=%%B"
  if "%%A"=="SAGA_NODE_ID" set "SAGA_NODE_ID=%%B"
  if "%%A"=="SAGA_NODE_TOKEN" set "SAGA_NODE_TOKEN=%%B"
)
if not defined PGPASSWORD (echo POSTGRES_PASSWORD is missing from .env.& exit /b 1)
if not defined PGUSER (echo POSTGRES_USER is missing from .env.& exit /b 1)
if not defined PGDATABASE (echo POSTGRES_DB is missing from .env.& exit /b 1)
if not defined POSTGRES_APP_USER (echo POSTGRES_APP_USER is missing from .env.& exit /b 1)
if not defined POSTGRES_APP_PASSWORD (echo POSTGRES_APP_PASSWORD is missing from .env.& exit /b 1)
if not defined SAGA_OPERATOR_ID (echo SAGA_OPERATOR_ID is missing from .env.& exit /b 1)
if not defined SAGA_OPERATOR_TOKEN (echo SAGA_OPERATOR_TOKEN is missing from .env.& exit /b 1)
if not defined SAGA_NODE_ID (echo SAGA_NODE_ID is missing from .env.& exit /b 1)
if not defined SAGA_NODE_TOKEN (echo SAGA_NODE_TOKEN is missing from .env.& exit /b 1)

set "TD_MIGRATIONS_ROOT=%ROOT:\=/%/infra/postgres/migrations"
"%PSQL%" -X -v ON_ERROR_STOP=1 -h 127.0.0.1 -p 15432 -U "%PGUSER%" -d "%PGDATABASE%" -f "%ROOT%\scripts\native\run-postgres-migrations.sql"
exit /b %ERRORLEVEL%